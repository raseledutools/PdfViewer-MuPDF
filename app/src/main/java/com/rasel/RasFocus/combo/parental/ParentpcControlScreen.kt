...
@Composable
// FIX: renamed from SpecificAppsSection — the same package
// (combo.parental) also has a SpecificAppsSection(Device, MainViewModel,
// (String)->Unit) in ChildPhoneControlScreen.kt with a different
// signature. Kotlin's overload resolution treated both as candidates for
// the same call and failed with "None of the following functions can be
// called". Prefixing this PC-specific one avoids the collision.
...
@Composable
// FIX: renamed from SelfControlFeaturesSection for the same reason as
// PcSpecificAppsSection above — ChildPhoneControlScreen.kt has a
// same-named function with a different signature in this package.
